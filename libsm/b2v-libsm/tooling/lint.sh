...
lint() {
    ...
    diffMarkers=$(echo "$filenames" | xargs grep -E '^[>|<]{7} ' -H -I || true)
    ...
    for file in $filenames; do
            ...
            if [ "$interactive" == true ] || [ "$acceptAllLinter" == true ]; then
                ...
                case "$lint_response" in
                    [yY][eE][sS]|[yY])
                        ...
                        ;;
                    *)
                        msg "You have chosen to keep your file, ill-formatted as it is...good luck.\n"
                        ((ignored += 1))
                        ;;
                esac
            else
        ...
        if test "$(tail -c1 "$file")"; then
            msg "No newline at end of $file. Fix your editor, in vscode it's called 'insert final newline'."
            if [ "$interactive" == true ] || [ "$acceptAllLinter" == true ]; then
                if [ "$acceptAllLinter" == true ]; then
                    lint_response=y
                else
            ...
            if [ "$interactive" == true ] || [ "$acceptAllLinter" == true ]; then
                ...
                case "$lint_response" in
                    [yY][eE][sS]|[yY])
                        ...
                        if [  "$ret" -ne 0 ]; then
                            msg "an error occurred when fixing typos"
                            msg "probably there was more than once choice so you have to manually fix it"
                            fail=true
                        fi
                        ;;
                    *)
    ...
    if ! command -v clang-format >/dev/null 2>&1; then
        ...
        if [[ "$filenames" == *".h"* || "$filenames" == *".c"* ]]; then
            for file in $(echo -e "$filenames" | grep -E '(\.h$|\.c$)'); do
                patch="$file.patch"
                rm -f "$patch"

                if [ "$staged" == true ]; then
                    git diff -U0 --no-color --staged "$file" | clang-format-diff -style file:./tooling/clang-format -p1 > "$patch"
                else
                    lintedFile=$(mktemp)
                    trap '{ rm -f "$lintedFile"; }' EXIT
                    clang-format --style=file:./tooling/clang-format "$file" > "$lintedFile"
                ...
                case "$lint_response" in
                    [yY][eE][sS]|[yY])
                        ...
                        if [  "$ret" -ne 0 ]; then
                            msg "an error occurred when applying clang-format patch $patch: $ret"
                            fail=true
                        else
                            rm "$patch"
                        fi
    ...
    # It's easier.
    todoMarkers=$(echo "$filenames" | sed 's/tooling.lint\.sh//' | xargs grep 'TODO' -n -H -I | grep -v TODOLIST || true)
    ...
    if [[ -n "$todoMarkers" ]]; then
        ...
        colorMessage "$todoColour" "WARNING: You have TODOs. Please fix them or make a ticket and do '// FT: BV-xxx whatever'"
        msg "\n$todoMarkers\n"
    ...
    ###################### submodules on release tag  ###### ###################
    # shellcheck disable=SC2016
    if ! git submodule foreach --quiet 'if ! ((git describe || (git fetch >/dev/null && git describe)) | grep -q "^v[0-9]*\(.[0-9]*\(.[0-9]*\)\?\)\?$"); then echo "$name version $(git describe) not a release tag"; exit 1; fi'; then
        redmsg "a submodule is not on a release branch"
        fail=true
    fi
